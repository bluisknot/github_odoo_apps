...
SUPPRESS_NOTIFICATIONS_CTX = {"suppress_notifications": True}
...
class BasicModelOpsNotification(models.AbstractModel):
    ...
    def write(self, vals):
        ...
        if self._do_notify(self and result):
            ...
            non_computed = [
                fname
                for fname in vals
                if (not self._fields[fname].compute and not self._fields[fname].related)
                or not self._fields[fname].readonly
            ]
            non_ignored = [
                f" * Field: {fname} - New value: {vals.get(fname)}"
                for fname in non_computed
    ...
    @api.model
    def _do_notify(self, result_or_records):
        return (
            result_or_records
            and not self._context.get("suppress_notifications", None)
            and not self._context.get("install_mode", None)
        )
    ...
    def _perform_notification(
        ...
        message=None,
        modified_fields_and_values=None,
        as_action=False,
        title=None,
            ...
            if not as_action:
                ...
                if modified_fields_and_values:
                    _logger_message_extension = (
                        '\nSince a "write" operation was trigger here are the modified fields and new values:\n'
                    )
                    modified_fields_and_values = "\n".join(modified_fields_and_values)
                    _logger_message = f"{_logger_message}{_logger_message_extension}{modified_fields_and_values}"
...
def suppress_notifications():
    ...
    return with_specific_context(SUPPRESS_NOTIFICATIONS_CTX)
...
def notifications_suppressor(model_instance):
    ...
    yield model_instance.with_context(SUPPRESS_NOTIFICATIONS_CTX)
